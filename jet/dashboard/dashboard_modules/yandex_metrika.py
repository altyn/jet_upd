...
class YandexMetrikaBase(DashboardModule):
    ...
    def counter_attached(self):
        if self.access_token is None:
            self.error = mark_safe(_('Please <a href="%s">attach Yandex account and choose Yandex Metrika counter</a> '
                                     'to start using widget') % reverse('jet-dashboard:update_module',
                                                                        kwargs={'pk': self.model.pk}
                                                                        )
                                   )
            return False
        elif self.counter is None:
            self.error = mark_safe(_('Please <a href="%s">select Yandex Metrika counter</a> to start using widget') %
        ...
        if self.counter_attached():
            ...
            if exception is not None:
                ...
                if isinstance(exception, HTTPError) and exception.code == 403:
                    error += _(' Try to <a href="%s">revoke and grant access</a> again') % \
                             reverse('jet-dashboard:update_module', kwargs={'pk': self.model.pk})
